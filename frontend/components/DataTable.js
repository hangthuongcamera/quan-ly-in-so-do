export const DataTable = ({ columns, data, showActions = true }) => {
    ...
    const header = columns.map(col => `
        <th class="p-4 text-left text-sm font-semibold text-text dark:text-gray-300 uppercase tracking-wider">${col.label}</th>
    `).join('');
    ...
    const rows = data.map(row => `
        <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
            ${columns.map(col => `
                <td class="p-4 text-sm text-muted">${row[col.key] || ''}</td>
            `).join('')}
            ${showActions ? `
